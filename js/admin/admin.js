if (localStorage.getItem('userLogued') !== null && JSON.parse(localStorage.getItem('userLogued')).email === "admin@admin") {
    if (localStorage.getItem('showAdminTypeVehicle') === "vans") {
    ...
    else if (localStorage.getItem('showAdminTypeVehicle') === "users") {
        searchUsersForAdmin();
    }
    else {
    ...
    function searchVehiclesForAdmin(vehicleType) {
        ...
        fetch("../../php/adminSearch.php", {
            ...
            let listVehicles = document.createElement('ul');
            listVehicles.setAttribute("class", "list-group");
            for (let i = 0; i < data.length; i++) {
                let buttonUpdate = "";
                let listItem = document.createElement('li');
                listItem.setAttribute("id", data[i]._id.$oid);
                listItem.setAttribute("class", "list-group-item p-3 fs-6");
                ...
                listItem.innerHTML = "<div class='row'><span class='col-7'><u><b>Modelo:</b></u> " + data[i].modelo + " &rarr; <u><b>Segunda Mano:</b></u> " + data[i].segunda_mano + "</span>"
                    + buttonUpdate
            ...
            document.getElementById("infoContainerAdmin").appendChild(listVehicles);
    ...
    function searchUsersForAdmin() {
        document.getElementById("infoContainerAdmin").innerHTML = "";
        fetch("../../php/adminUsersSearch.php", {
            method: 'POST'
        }).then(function (respuesta) {
            return respuesta.json();
        }).then(function (data) {
            ...
            listUsers.setAttribute("class", "list-group");
            for (let i = 0; i < data.length; i++) {
                let listItem = document.createElement('li');
                listItem.setAttribute("id", data[i]._id.$oid);
                listItem.setAttribute("class", "list-group-item p-3 fs-5");

                listItem.innerHTML = "<u><b>Nombre de usuario:</b></u> " + data[i].nombre + " &rarr; <u><b>Email:</b></u> " + data[i].email ;

                listUsers.appendChild(listItem);

            }
            document.getElementById("infoContainerAdmin").appendChild(listUsers);
        }).catch(function (ex) {
            console.log("Error", ex.mesagge)
        });
    }
